// RHU Katipunan Blood Donation System - Enhanced Responsive Version
...
function saveToLocalStorage() {
    try {
        ...
        // Store in memory only - no localStorage usage
        window.bloodDonationData = data;
    } catch (e) {
...
function loadFromLocalStorage() {
    try {
        if (window.bloodDonationData) {
            const data = window.bloodDonationData;
            users = data.users || {};
            schedules = data.schedules || [];
            appointments = data.appointments || [];
            notifications = data.notifications || { admin: [], donor: [] };
            messages = data.messages || [];
            deletedItems = data.deletedItems || { schedules: [], appointments: [], donors: [] };
            return true;
        }
    } catch (e) {
...
function closeSignupForm() {
    ...
    ['signupUsername', 'signupPassword', 'signupConfirmPassword', 'signupName', 'signupEmail', 'signupContact'].forEach(id => {
        const el = qs(id);
        if (el) el.value = '';
    });
}
...
function createAccount() {
    ...
    if (!username || !password || !confirmPassword || !fullname || !email || !contact) {
...
/* ---------- ADMIN: Schedules / Appointments / Donors ---------- */
...
function loadAppointmentsList() {
    ...
    if (pendingContainer) {
        if (pending.length === 0) {
            ...
            let html = '<div style="overflow-x: auto;"><table><thead><tr><th>Name</th><th>Blood Type</th><th>Donation #</th><th>Bags</th><th>Date</th><th>Time</th><th>Actions</th></tr></thead><tbody>';
            pending.forEach(apt => {
                const sched = schedules.find(s => s.id === apt.scheduleId);
                html += `
                    <tr>
                        <td>${escapeHtml(apt.name)}</td>
                        <td><span class="badge badge-danger">${escapeHtml(apt.bloodType)}</span></td>
                        <td><span class="badge badge-info">${apt.donationNumber}${getOrdinalSuffix(apt.donationNumber)}</span></td>
                        <td>${apt.numBags}</td>
                        <td>${sched ? formatDate(sched.date) : 'N/A'}</td>
                        <td>${sched ? escapeHtml(sched.time) : 'N/A'}</td>
                        <td class="action-buttons">
                            <button onclick="approveAppointment(${apt.id})" class="btn-sm btn-success"><i class="fas fa-check"></i> Approve</button>
                            <button onclick="rejectAppointment(${apt.id})" class="btn-sm btn-danger"><i class="fas fa-times"></i> Reject</button>
                        </td>
                    </tr>
                `;
            });
    ...
    if (allContainer) {
        if (appointments.length === 0) {
            ...
            let html = '<div style="overflow-x: auto;"><table><thead><tr><th>Name</th><th>Blood Type</th><th>Donation #</th><th>Bags</th><th>Contact</th><th>Date</th><th>Status</th><th>Actions</th></tr></thead><tbody>';
            appointments.forEach(apt => {
                ...
                html += `
                    <tr>
                        ...
                        <td class="action-buttons">
                            ${apt.status === 'approved' ? `<button onclick="completeAppointment(${apt.id})" class="btn-sm btn-info"><i class="fas fa-check-circle"></i> Complete</button>` : ''}
                            <button onclick="viewAppointmentDetails(${apt.id})" class="btn-sm btn-info"><i class="fas fa-eye"></i> View</button>
                            <button onclick="deleteAppointment(${apt.id})" class="btn-sm btn-danger"><i class="fas fa-trash"></i> Delete</button>
                        </td>
                    </tr>
                `;
            });
...
function getDonationNumber(username) {
    const user = users[username];
    if (!user) return 1;
    const completed = appointments.filter(a => a.donorUsername === username && a.status === 'completed').length;
    return (user.previousDonations || 0) + completed + 1;
}
...
DONATION DETAILS:
Donation Number: ${apt.donationNumber}${getOrdinalSuffix(apt.donationNumber)} Time
Number of Bags: ${apt.numBags}
Medical History: ${apt.medicalHistory || 'None'}
${apt.lastDonationDate ? 'Last Donation: ' + formatDate(apt.lastDonationDate.split('T')[0]) : 'First Time Donor'}
...
function loadDonorsList() {
    ...
    let html = '<div style="overflow-x: auto;"><table><thead><tr><th>Name</th><th>Username</th><th>Email</th><th>Contact</th><th>Total Donations</th><th>Pending</th><th>Last Donation</th><th>Registered</th><th>Actions</th></tr></thead><tbody>';
    registeredDonors.forEach(donor => {
        const donorAppts = appointments.filter(a => a.donorUsername === donor.username);
        const completed = donorAppts.filter(a => a.status === 'completed').length;
        const pending = donorAppts.filter(a => a.status === 'pending').length;
        const totalDonations = (donor.previousDonations || 0) + completed;
        const lastCompleted = donorAppts.filter(a => a.status === 'completed').pop();
        ...
        html += `
            <tr>
                ...
                <td>${escapeHtml(donor.contact)}</td>
                <td><span class="badge badge-info">${totalDonations}</span></td>
                <td>${pending > 0 ? `<span class="badge badge-warning">${pending}</span>` : '-'}</td>
                <td>${lastCompleted ? formatDate(lastCompleted.completedDate.split('T')[0]) : 'Never'}</td>
                <td>${formatDate(donor.registeredAt.split('T')[0])}</td>
                <td class="action-buttons">
...
/* ---------- DONOR: Dashboard / Booking / Appointments ---------- */
...
function checkEligibility() {
    const checks = {
        ...
        lastDonation: qs('lastDonation').checked,
        ...
        notPregnant: qs('notPregnant').checked,
        notMenstruating: qs('notMenstruating').checked,
        noRecentPregnancy: qs('noRecentPregnancy').checked,
        noCovid: qs('noCovid').checked,
        noCovidSymptoms: qs('noCovidSymptoms').checked
    };
    ...
    if (allChecked) {
        ...
        if (!checks.lastDonation) reasons.push('Must wait 3 months from last donation');
        ...
        if (!checks.noCovid) reasons.push('Must not have had COVID-19 in last 14 days');
        if (!checks.noCovidSymptoms) reasons.push('Must not have COVID-19 symptoms');
...
function toggleDonationFields() {
    ...
    const lastContainer = qs('lastDonationContainer');
    if (lastContainer) lastContainer.style.display = hasDonated === 'yes' ? 'block' : 'none';
}
...
function proceedToBooking() {
    ...
    const donationNumber = getDonationNumber(currentUser.username);
    
    const donationNumberField = qs('donationNumberDisplay');
    if (donationNumberField) {
        donationNumberField.textContent = `This will be your ${donationNumber}${getOrdinalSuffix(donationNumber)} donation`;
    }
    ...
    if (completedDonations.length > 0) {
        ...
        const lastDonationDate = qs('lastDonationDate');
        if (lastDonationDate && lastDonation.completedDate) {
            lastDonationDate.value = lastDonation.completedDate.split('T')[0];
        }
    }
...
function submitBooking() {
    ...
    if (!name || !age || !gender || !bloodType || !contact || !email || !address || !emergencyName || !emergencyContact) {
    ...
    const fields = ['donorName','donorAge','donorGender','donorBloodType','donorContact','donorEmail','donorAddress','numBags','medicalHistory','emergencyName','emergencyContact','lastDonationDate','hasDonatedBefore'];
    fields.forEach(f => { 
        const field = qs(f);
    ...
    closeBookingModal();
    alert('Appointment submitted successfully! This will be your ' + donationNumber + getOrdinalSuffix(donationNumber) + ' donation. Please wait for admin approval.');
    loadDonorSchedules();
    loadDonorAppointments();
    updateAdminNotifications();
...
function loadDonorAppointments() {
    ...
    let html = '<div style="overflow-x: auto;"><table><thead><tr><th>Date</th><th>Time</th><th>Staff</th><th>Blood Type</th><th>Donation #</th><th>Bags</th><th>Status</th><th>Actions</th></tr></thead><tbody>';
...
/* ---------- ENTER KEY to Login & Send Message ---------- */
